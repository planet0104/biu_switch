...
fn main() -> ! {
    ...
    if let (Some(mut p), Some(cp)) = (pac::Peripherals::take(), Peripherals::take()) {
        ...
        hprintln!("unlocked flash..");
        let write_data = [0xC0_u8, 0xFF_u8, 0xEE_u8, 0x00_u8];
        NorFlash::erase(&mut unlocked_flash, 0x0800F000, 0x0800F000 + write_data.len() as u32).unwrap();
        let ok = NorFlash::write(&mut unlocked_flash, 0x0800F000, &write_data);
        hprintln!("write ok:{}", ok.is_ok());
