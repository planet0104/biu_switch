...
// 全局变量用于在中断和主循环间共享状态
static BUTTON_PRESSED: cortex_m::interrupt::Mutex<core::cell::RefCell<bool>> = 
...
fn main() -> ! {
    ...
    if let (Some(mut p), Some(cp)) = (pac::Peripherals::take(), Peripherals::take()) {
        ...
        // 启用外部中断
        let mut exti = p.EXTI;
        gpio_button.make_interrupt_source(&mut exti);
        button.trigger_on_edge(&mut exti, stm32f1xx_hal::gpio::Edge::FALLING);
