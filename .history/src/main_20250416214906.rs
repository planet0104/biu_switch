...
fn main() -> ! {
    ...
    // 默认10分钟关机
    let mut delay_poweroff_five_minutes = 2;
    ...
    if let (Some(mut p), Some(cp)) = (pac::Peripherals::take(), Peripherals::take()) {
        ...
        loop {
            ...
            if let Ok(true) = gpio_button.is_low(){
                ...
                if delay_poweroff_five_minutes > 6{
                    delay_poweroff_five_minutes = 0;
                }else{
                    delay_poweroff_five_minutes += 5;
                }
                let five_sec = delay_poweroff_minutes/5;
                if five_sec == 0{
                    beep(&mut gpio_beep, &mut delay, 1);
                }else{
                    beep(&mut gpio_beep, &mut delay, five_sec);
                }
