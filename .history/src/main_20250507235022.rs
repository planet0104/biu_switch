...
fn main() -> ! {
    ...
    if let (Some(mut p), Some(cp)) = (pac::Peripherals::take(), Peripherals::take()) {
        ...
        let mut gpio_speaker_volume = cortex_m::interrupt::free(move |cs| gpioa.pa0.into_analog(cs));
        ...
        loop {
            ...
            // let speaker_volume: u16 = adc.read(&mut gpio_speaker_volume).unwrap_or(0);
            // let volt_float: f32 = volt_val as f32 / 4.092;
            // let volt_int = volt_float as i32;
            // hprintln!("{}", speaker_volume);
            // // 不接音箱的时候，A0读取到:2966~3004 电压表测量:2.39V
            // // 连接音箱时:
