...
use stm32f4xx_hal::{self as hal, i2s::{stm32_i2s_v12x::{driver::{ClockPolarity, DataFormat, I2sDriver, I2sDriverConfig}, marker::{Master, Philips, Receive}, transfer::{I2sTransfer, I2sTransferConfig}}, I2s}, pac::SPI2};
...
fn main() -> ! {
    if let (Some(dp), Some(_cp)) = (
        ...
        let transfer_config = I2sTransferConfig::new_master()
            .receive()
            .standard(Philips)
            .data_format(NEW_FMT::Data16Channel16)
            .master_clock(true)
            .request_frequency(16_000);
