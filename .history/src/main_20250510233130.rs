...
fn main() -> ! {
    if let (Some(dp), Some(_cp)) = (
        ...
        delay.delay_ms(1000*6);
        //关机，切换到push_pull_output，并设置低电平
        let mut po = power_control.into_push_pull_output();
        po.set_low();
        // power_control.set_high();
        delay.delay_ms(1500);
        //切换回阻塞模式
        let mut po = po.into_floating_input();
...
//开机后，电源按钮引脚默认是floating_input，此时断开
fn power_off(pin: &PA5<DefaultMode>, delay:&Delay<TIM5, 1000000>, duration_ms: u32){
    let _ = pin.set_low();
    // delay.delay_ms(100u16);
    // let _ = pin.set_low();
}
