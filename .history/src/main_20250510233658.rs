...
// 切换到蓝牙模式(继电器io连接低电平，触发继电器)
fn switch_to_bluetooth(pin: PA5<DefaultMode>, delay:&mut Delay<TIM5, 1000000>, duration_ms: u32) -> PA5<DefaultMode>{
    let mut pin_output = pin.into_push_pull_output();
    pin_output.set_low();
    delay.delay_ms(duration_ms);
    //切换回阻塞模式
