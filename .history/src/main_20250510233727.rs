...
use stm32f4xx_hal::{
    self as hal, gpio::{DefaultMode, Input, Output, Pin, PA5, PA6}, i2s::{
        stm32_i2s_v12x::{
...
fn switch_to_bluetooth(pin: PA7<DefaultMode>, delay:&mut Delay<TIM5, 1000000>, duration_ms: u32) -> fn switch_to_bluetooth(pin: PA7<DefaultMode>, delay:&mut Delay<TIM5, 1000000>, duration_ms: u32) -> PA5<DefaultMode>{
<DefaultMode>{
    let mut pin_output = pin.into_push_pull_output();
    pin_output.set_low();
    delay.delay_ms(duration_ms);
